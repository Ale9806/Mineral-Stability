...
species <- species(c("gibbsite", "muscovite", "kaolinite", "pyrophyllite", "K-feldspar"))
...
plot_stability_segment <- ggplot()+
  geom_vline(xintercept = -4.675) + # add vertical line for gibbsite - kaolinite equilibrium
...
```{r}
gibbsite_intersect_KH <- -3*-4.68-9.97 # solve for Y coordinate of the two intersecting equilibrium lines and assign it to a numeric value
gibbsite_intersect_KH # print that value
```
...
plot_stability_segment4 <- ggplot()+
  geom_segment(aes(x = -4.68, y = gibbsite_intersect_KH, xend = -4.68, yend = -3))+ # gibbsite - kaolinite equilibrium line
  geom_segment(aes(x = -4.68, y = gibbsite_intersect_KH, xend = gibbsite_musc_edge_H4SiO4, yend = 6))+ # gibbsite - muscovite equilibrium line
  scale_x_continuous(limits = c(-6, -2), expand = c(0, 0), name = TeX("log $\\left[$H$_4$SiO$_4$$\\right]$"))+
   scale_y_continuous(limits = c(-3, 6), expand = c(0, 0), name = TeX("log $\\left[$ $\\frac{K$^+$}{H$^+$}$ $\\right]$"))+
  theme_bw()+
  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank())+
