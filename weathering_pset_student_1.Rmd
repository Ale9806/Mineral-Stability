...
**Answer 2** since gibbsite is a hydoxide (compared to the other minerals in the diagram, which are silicates), it is consistent with Le Châtelier's Principle that gibbsite would be the stable mineral at the lowest $\left[\text{H}_4\text{SiO}_4\right]$.
...
Rearrange as Y=mX+b
$\log\frac{\left[\text{K}^+\right]}{\left[\text{H}^+\right]}=-3\log\left[\text{H}_{4}\text{SiO}_{4}\right]-5.9325$
...
**Answer 4** Microcline and gibbsite can never be in equilibrium with each other in this system. The gibbsite that will form here is metastable, meaning it is only temporarily stable, but as the water chemistry approaches equilibrium with microcline, the gibbsite will be consumed.
...
to do:
  + Format (TeX slows it down too much...)
    + Space between plots
    + Legend formatting
    + Title
  + Fix steps to be more representative
  + Add equivalent of geom_segments
  + Seb's class example (plotly with markers instead of the bars) might allow things to render better?
...
plot_reactions_ABCDE_anim <- ABCDE %>%
  ...
  arrange(log_H4SiO4) %>%
  plot_ly() %>%
#  add_segments(x = gibb_intersect_H4SiO4, y = gibb_intersect_KH, xend = -4.68, yend = -3) %>% 
  ...
  layout(title = "Concentrations",
         xaxis = list(title = "log[H4SiO4]", range = c(-6, -3)), # plotly::TeX("log \\left[H_4SiO_4\\right]")
         yaxis = list(title = "log[K+/H+]", range = c(-3, 5))) # plotly::TeX("log \\left[ \\frac{K^+}{H^+} \\right]")

plot_minerals_rxns_ABCDE_anim <- ABCDE %>%
  dplyr::slice(seq(from = 1, to = nrow(ABCDE), by = 100)) %>% # select only every 100th point. Default "slice" is from CHNOSZ package
  gather(key = "phase", value = "moles", mol_gibbs, mol_kaol, mol_musc) %>%
  plot_ly() %>%
  add_trace(x = ~phase,
            y = ~moles,
            type = 'bar',
            ...
            color = ~phase) %>%
  layout(title = "Concentrations",
         xaxis = list(title = "Reaction progress"), # plotly::TeX("\\text{reaction progress, } \\xi")
         yaxis = list(title = "Moles mineral in system", range = c(0, 50*10^-6))) # plotly::TeX("\\text{moles mineral in system}"




subplot(plot_reactions_ABCDE_anim, plot_minerals_rxns_ABCDE_anim, nrows = 1, titleX = T, titleY = T) %>%
  animation_opts(frame = 200, transition = 0, redraw = TRUE) #%>%
#  config(mathjax = 'cdn')
```


Slicing this up better:
...
plot_minerals_ABCDE_test <- ABCDE %>%
  ...
  geom_line(aes(x=steps, y=mol_kaol, color = "moles kaolinite"))+
  scale_y_continuous(name = "moles mineral in system")+
  scale_x_continuous(name = plotly::TeX("reaction progress, $\\xi$"))+
  theme_bw() %>%
  config(mathjax = 'cdn')


plot_minerals_ABCDE_test %>% ggplotly() %>%
  layout(
    xaxis = list(
      title = plotly::TeX("\\text{reaction progress, } \\xi")),
    yaxis = list(
      title = plotly::TeX("\\text{moles mineral in system}"))) %>%
  config(mathjax = 'cdn')



ply <- plot_minerals_ABCDE_test %+% aes(frame = steps) %>% ggplotly() + layout(
    xaxis = list(
      title = plotly::TeX("\\text{reaction progress, } \\xi")),
    yaxis = list(
      title = plotly::TeX("\\text{moles mineral in system}"))) %>%
...
Now try ggplotly with the more complicated phase diagram:

```{r}
# # This mostly works, but R can't handle it.

# plot_ABCDE_test <- ABCDE %>% ggplot(aes(x=log_H4SiO4, y = log_K_H_ratio, color = steps))+ # pass the dataframe to ggplot and tell it what to use for x and y
#   geom_segment(aes(x = gibb_intersect_H4SiO4, y = gibb_intersect_KH, xend = -4.68, yend = -3), color = "black", size = 0.3)+
#   geom_segment(aes(x = gibb_intersect_H4SiO4, y = gibb_intersect_KH, xend = gibb_musc_edge_H4SiO4, yend = 6), color = "black", size = 0.3)+
#   geom_segment(aes(x = gibb_intersect_H4SiO4, y = gibb_intersect_KH, xend = kaol_musc_micr_eq_point_H4SiO4, yend = kaol_musc_micr_eq_point_KH), color = "black", size = 0.3)+
#   geom_segment(aes(x = kaol_musc_micr_eq_point_H4SiO4, y = kaol_musc_micr_eq_point_KH, xend = musc_micr_edge_H4SiO4, yend = musc_micr_edge_KH), color = "black", size = 0.3)+
#   geom_segment(aes(x = kaol_musc_micr_eq_point_H4SiO4, y = kaol_musc_micr_eq_point_KH, xend = -2, yend = kaol_micr_edge_KH), color = "black", size = 0.3)+
#   scale_x_continuous(limits = c(-6, -2), expand = c(0, 0), name = plotly::TeX("log \\left[H_4SiO_4\\right]"))+
#    scale_y_continuous(limits = c(-3, 6), expand = c(0, 0), name = plotly::TeX("log \\left[ \\frac{K^+}{H^+} \\right]"))+
#   theme_bw()+
#   theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank())+
#  theme(axis.title.y = element_text(angle = 0, vjust = .5, size = 11))+
#  annotate("text", y = 4, x= -5.4, label = "gibbsite", size = 4)+
...
#  annotate("text", y = -2, x= -3, label = "kaolinite", size = 4)+
#  geom_point(size=1, alpha =1) +
#   scale_color_continuous(name = plotly::TeX("reaction progress, \\xi"))+ # add legend for reaction progress
#  theme(axis.title.y = element_text(angle = 0, vjust = .5, size = 11))+
#   annotate("text", y = -2.4, x=-5.85, label = "A", size = 6)+ # annotate point A
#   annotate("text", y = -1.3, x=-4.55, label = "B", size = 6)+ # annotate point B
#   annotate("text", y = -.8, x=-4.8, label = "C", size = 6)+ # annotate point C
