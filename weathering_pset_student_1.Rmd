...
Mineral Stability Diagrams tell us what minerals are stable and what are not given a specific water chemistry. When a mineral is unstable it is a __reactant_ that will _weather_ into a _product_. 
...
\fra
The x-axis of the diagram is $log[H_{4}SiO_{4}]$, which is silicic acid. The Y-axis in this case will be $log \frac {[K^{+}]}{[H^{+}]}$. While the X-axis will always be silicic acid, the Y-axis can have different cations depending on the mineral system of interest.
You should have already derived all the weathering reactions that are relevant for the phase diagram we are building. For your reference, they are listed below.


$2 KAlSi_{3}O_{8} + 9 H_{2}O + 2 H^{+} \leftrightharpoons Al_{2}Si_{2}O_{5}(OH)_{4} + 2 K^{+} + 4H_{4}SiO_{4},    Keq = 10^{-5.21}$ 
$KAlSi_{3}O_{8} + H^{+} + 7H_{2}O \leftrightharpoons Al(OH)_{3} + 2K^{+} + 3H_{4}SiO_{4}. Keq = $
$KAlSi_{3}O_{8} + 2Al(OH)_{3} + H^{+}  \leftrightharpoons 1.5Al_{2}Si_{2}O_{5}(OH)_{4} + K^{+} + 0.5H_{2}O, Keq = $
$2KAlSi_{3}O_{8} + 2H^{+} + 9H_{2}O \leftrightharpoons Al_{2}Si_{2}O_{5}(OH)_{4} + 2K^{+} + 4H_{4}SiO_{4}, Keq = $
...
**Question 2**: Which axis tells you about pH? How do you know this? 
...
$[H_{4}SiO_{4}] = 1e^{-6}$
$[K^{+} = 1e^{-4}] $ Note that this is the same as the moles of microcline
$pH = 4$
...
When ploting the initial water chemistry point, you should notice that it sits within the gibbsite field. This is not in equilibrium with our mineral, microcline. Meaning, microcline (K-feldspar) is NOT stable in this water chemistry. 
...
Now our initial reaction conditions has changed. Now, we have some gibbsite in our system that was produced by K-feldspar weathering. However, not all of the K-feldspar could be weathered into gibbsite before the water chemistry has moved to a new stability phase. Before we can continue our path, the gibbsite which is now unstable, must be reacted. Think of the phase boundary as a gate. In our case, the gate will not let any gibbsite through, only kaolinite. So, all of this gibbsite must be reacted into kaolinite before we can continue evolving the water chemistry.
