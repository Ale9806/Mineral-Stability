...
```{r}
gibbsite_musc_edge_H4SiO4 <- -(6+9.97)/3 # solve for X coordinate where gibbsite-muscovite equilibrium line exits the plot and assign it to a numeric value
gibbsite_musc_edge_H4SiO4  # print that value
```


```{r fig.height=4, fig.width=4}
plot_stability_segment4 <- ggplot()+
  geom_segment(aes(x = gibb_intersect_H4SiO4, y = gibbsite_intersect_KH, xend = gibb_intersect_H4SiO4, yend = -3))+ # gibbsite - kaolinite equilibrium line
  geom_segment(aes(x = gibb_intersect_H4SiO4, y = gibbsite_intersect_KH, xend = gibbsite_musc_edge_H4SiO4, yend = 6))+ # gibbsite - muscovite equilibrium line
  scale_x_continuous(limits = c(-6, -2), expand = c(0, 0), name = TeX("log $\\left[$H$_4$SiO$_4$$\\right]$"))+
   scale_y_continuous(limits = c(-3, 6), expand = c(0, 0), name = TeX("log $\\left[$ $\\frac{K$^+$}{H$^+$}$ $\\right]$"))+
  theme_bw()+
  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank())+
...
plot_stability_segment5 <- ggplot()+
  geom_segment(aes(x = gibb_intersect_H4SiO4, y = gibbsite_intersect_KH, xend = -4.68, yend = -3))+
  geom_segment(aes(x = gibb_intersect_H4SiO4, y = gibbsite_intersect_KH, xend = gibbsite_musc_edge_H4SiO4, yend = 6))+
  geom_segment(aes(x = gibb_intersect_H4SiO4, y = gibbsite_intersect_KH, xend = kaol_musc_micr_eq_point_H4SiO4, yend = kaol_musc_micr_eq_point_KH))+ # muscovite - kaolinite equilibrium line
...
The lines on the mineral stability digram describe regions where the two minerals are in equilibrium.
